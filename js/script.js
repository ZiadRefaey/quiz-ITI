...
nameInput.addEventListener("input", function ({ target }) {
  checkInput(
    ...
    nameErr,
    nameReg,
    "Name must be 3–20 English letters only. Spaces are allowed but not at the beginning or end.",
    "Correct user name",
  );
});
emailInput.addEventListener("input", function ({ target }) {
  checkInput(
    ...
    emailErr,
    emailReg,
    "Please enter a valid email address that contains @ and ends with .com",
    "correct email",
  );
});
pass.addEventListener("input", function ({ target }) {
  checkInput(
    ...
    passErr,
    passReg,
    "Password must be 6–12 characters (letters and numbers only, no spaces or special characters).",
    "password accenpted",
  );
});
